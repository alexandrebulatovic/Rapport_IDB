...
Ils ont également un rôle de générateur de SQL, généralement "simple" ou qui diffère selon le SGBD connecté, contrairement aux classes métiers qui fournissent du SQL qui fonctionne sur tous les SGBD.
...
En pratique, cela prendrait beaucoup trop de temps.
En revanche, l'application est conçue pour s'adapter à un nouvel SGBD sans modifier de code existant (ou presque), il "suffit" d'en ajouter pour utiliser un nouvel SGBD.
...
De ce qu'il faut comprendre, c'est qu'elle est toujours appelée depuis un contrôleur (lui-même passant par les façades).
En plus de cela, la fabrique possède des dépendances de stéréotypes \textit{create} vers les autres paquetages :
\begin{itemize}
\item \textit{gui} : \underline{pas} pour fabriquer des IHM, mais des classes imposées par Java pour les faire fonctionner, avec un comportement différent selon le SGBD connecté.
\item \textit{manager} : pour créer des DAO générant le code SQL adapté au SGBD connecté.
\end{itemize}
